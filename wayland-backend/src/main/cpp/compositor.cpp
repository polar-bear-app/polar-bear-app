...
struct wlr_surface *wlr_surface_from_resource(struct wl_resource *resource) {
    ...
    return static_cast<wlr_surface *>(wl_resource_get_user_data(resource));
}
...
static struct wlr_surface *surface_create(struct wl_client *client,
    ...
    struct wlr_surface *surface = static_cast<wlr_surface *>(calloc(1, sizeof(*surface)));
    if (!surface) {
...
struct wlr_compositor *compositor_from_resource(struct wl_resource *resource);
...
void wlr_surface_send_enter(struct wlr_surface *surface,
    ...
    surface_output = static_cast<wlr_surface_output *>(calloc(1, sizeof(*surface_output)));
...
static void handle_bounding_box_surface(struct wlr_surface *surface,
    ...
    struct bound_acc *acc = static_cast<bound_acc *>(data);
...
struct wlr_compositor *compositor_from_resource(struct wl_resource *resource) {
    assert(wl_resource_instance_of(resource, &wl_compositor_interface,
                                   &compositor_impl));
    return static_cast<wlr_compositor *>(wl_resource_get_user_data(resource));
}

static void compositor_bind(struct wl_client *wl_client, void *data,
                            uint32_t version, uint32_t id) {
    struct wlr_compositor *compositor = static_cast<wlr_compositor *>(data);
...
struct wlr_compositor *wlr_compositor_create(struct wl_display *display,
    ...
    struct wlr_compositor *compositor = static_cast<wlr_compositor *>(calloc(1,
                                                                             sizeof(*compositor)));
    if (!compositor) {
        return NULL;
    }
...
static bool surface_state_add_synced(struct wlr_surface_state *state, void *value) {
    void **ptr = static_cast<void **>(wl_array_add(&state->synced, sizeof(void *)));
    if (ptr == NULL) {
        return false;
    }
    *ptr = value;
...
static void *surface_state_remove_synced(struct wlr_surface_state *state,
                                         struct wlr_surface_synced *synced) {
    void **synced_states = static_cast<void **>(state->synced.data);
...
//    setenv("WAYLAND_DEBUG", "1", 1);  // Set WAYLAND_DEBUG=1 for verbose logs
    ...
    wlr_compositor_create(display, 6, nullptr);
...
void run_display() {
    ...
    setup_wayland_backend(socket_name);
